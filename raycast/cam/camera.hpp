...
private:
    ...
    // color ray_color(const ray& r, const hittable& space) const {
    //     hitted_surf tsurf;
    //     if (space.hit(r, interval(0, infty), tsurf)) 
    //         return 0.5 * (tsurf.normal_ + color(1,1,1));
    //         // return color(1,0,0);
    ...
    color ray_color(const ray& r, const hittable& world, int current_depth) const {
        ...
        if (world.hit(r, interval(0.001, infty), rec)) {
            // интервал делаем не от 0.0, а 0.001 
            vec3 direction = random_on_hemisphere(rec.normal_);
            return 0.5 * ray_color(ray(rec.point_, direction), world, current_depth - 1);
        }
